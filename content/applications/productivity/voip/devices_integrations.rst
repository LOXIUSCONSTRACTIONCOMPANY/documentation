...
app and through third-party apps as well. As long as the app is compatible with |SIP|, it can be
used to make and receive calls with Odoo |VOIP|.
...
Download the *Zoiper Lite* app from either the `Google's Play Store
<https://play.google.com/store/apps/details?id=com.zoiper.android.app>`_ or `Apple's App Store
<https://apps.apple.com/us/app/zoiper-lite-voip-soft-phone/id438949960>`_ (depending on the device's
operating system) to begin using *Zoiper Lite*. This article will cover setting up *Zoiper Lite* for
an iOS device while using :doc:`Axivox <axivox/axivox_config>` as the |VOIP| service provider, so
the experience may differ for other operating systems or |VOIP| service providers.

Once *Zoiper Lite* is installed on the mobile device, open the app and tap on :guilabel:`Settings`.
Navigate to :menuselection:`Accounts`, and tap on the :guilabel:`+ (plus)` icon to add an account.
...
If :guilabel:`Manual configuration` was tapped, then tap :guilabel:`SIP account`. From here, under
...
*Linphone* third-party alternative to Odoo |VOIP|. It is a free |VOIP| |SIP| dialer with voice,
...
using :doc:`Axivox <axivox/axivox_config>` as the |VOIP| service provider, so the experience may
differ for other operating systems or |VOIP| service providers.
