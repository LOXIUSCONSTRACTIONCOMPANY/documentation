...
.. list-table::
    ...
    * - :guilabel:`France - Factur-X integration with Chorus Pro`
      ...
      - Enables filling three optional fields required for using Chorus Pro, particularly when
    ...
    * - :guilabel:`France - VAT Anti-Fraud Certification for Point of Sale (CGI 286 I-3 bis)`
      - `10n_fr_pos_cert`
      - Compliance with the technical requirements of French regulation CGI art. 286, I. 3° bis,
        which outlines specific criteria for data related to sales to private individuals (B2C).
...
To send invoices to Chorus Pro, :doc:`install </applications/general/apps_modules>` the
:guilabel:`France - Factur-X integration with Chorus Pro` (`l10n_fr_facturx_chorus_pro`) module.

The :ref:`Peppol <e-invoicing/peppol>` network enables Odoo to send invoices directly to Chorus Pro.
To use this feature, :ref:`register <e-invoicing/peppol-registration>` with Peppol.

If you don’t already have a Chorus Pro account, go to the `Chorus Pro
<https://portail.chorus-pro.gouv.fr/aife_csm>`_ page and click :guilabel:`Créer un compte` to create
one.
...
To submit invoices through Chorus Pro, configure the relevant customers' contact form as follows:
...
#. Go to :menuselection:`Accounting --> Customers --> Invoices` and open or create the relevant
   invoice.
...
Once sent, the Peppol status of the invoice is updated to :guilabel:`Done`.
