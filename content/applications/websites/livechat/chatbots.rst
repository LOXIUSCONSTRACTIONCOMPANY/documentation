...
Before creating a new chatbot, the **Live Chat** application must first be installed on the
database. This can be done directly from the :menuselection:`Apps` application, by searching for
`Live Chat` in the :guilabel:`Search...` bar, and clicking :guilabel:`Install`.

Once the **Live Chat** application has been installed on the database, go to :menuselection:`Live
...
Enter a name in the :guilabel:`Chatbot Name` field and click on the :guilabel:`Edit Image`
:icon:`fa-pencil` :guilabel:`(pencil)` icon to add a photo.
...
chatbot detail page, and a :guilabel:`Create Script Steps` pop-up window appears.

This form must be filled out for each line of text (dialog) that the chatbot could potentially
